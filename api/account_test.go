...
import (
	...
	mockdb "github.com/avivbaron/simplebank/db/mock"
...
func TestListAccounts(t *testing.T) {
	pageIDRand := int32(util.RandomInt(1, 3))
	pageSizeRand := int32(util.RandomInt(1, 5))
	...
	testCases := []struct {
	...
	}{
		{
			name:     "OK",
			pageID:   pageIDRand,
			pageSize: pageSizeRand,
			buildStubs: func(store *mockdb.MockStore) {
				arg := db.ListAccountsParams{
					Limit:  pageSizeRand,
					Offset: (pageIDRand - 1) * pageSizeRand,
				}
				store.EXPECT().
			...
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				...
				requireBodyMatchAccountList(t, recorder.Body, accountList)
			},
		},
		{
			name:     "BadRequest",
...
func requireBodyMatchAccountList(t *testing.T, body *bytes.Buffer, list []db.Account) {
	data, err := io.ReadAll(body)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Equal(t, list, accountList)
}
